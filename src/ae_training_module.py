...
from src.constants import INPUT_DIR, OUTPUT_DIR, MODEL_DIR
...
class AETrainingModule:
    ...
    def fit(self, num_epochs: int = 100):
        ...
        for cur_epoch in (pbar_epoch := tqdm(range(num_epochs))):
            ...
            for i, (images, _) in (pbar := tqdm(enumerate(self.training_dataloader))):
                ...
                kl_loss = 1e-4 * self.model.kl_loss(mu, logvar)
    ...
    def save_model(self, network_name: str):
        MODEL_DIR.mkdir(parents=True, exist_ok=True)
        torch.save(self.model.state_dict(), MODEL_DIR / f"{network_name}.pth")

    def sample(self, num_samples: int = 10, epoch: int = 0):
        with torch.no_grad():
            samples = self.model.sample(num_samples)
            # Save as images
            torchvision.utils.save_image(samples, OUTPUT_DIR / f"epoch_{epoch}_samples.png", nrow=10)
