...
class SegmentationUNetTrainingModule(ABCTrainingModule):
    ...
    def compute_metrics(self, predictions, masks):
        ...
        for i in range(batches):
            vizualize_segmentation_predictions(self.output_path / f"epoch_{self.epoch}_mask_gt_p{i}.png", self.last_test_image_batch[6 * i:(i + 1) * 6].cpu().numpy(), torch.argmax(masks[length:][6 * i:(i + 1) * 6], dim=1).cpu().numpy(), torch.argmax(predictions[length:][6 * i:(i + 1) * 6], dim=1).cpu().numpy())
